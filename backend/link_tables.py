...
for index, row in dfs.df_dots_subcategories_filters.iterrows():
    ...
    for dots_sub_cat_id in dfs.df_dots_subcategories_filters.columns[2:]:
        ...
        if val == 1:
            val_id = dfs.df_dots_subcategories_keys.loc[dfs.df_dots_subcategories_keys['key'] == int(dots_sub_cat_id)]
            sub_category = val_id.iloc[0]['translation_key']
            org_sub_cats.append(sub_category)
            category = utils.get_cat_value(dfs.df_dots_categories_keys, dfs.df_dots_subcategories_keys, dots_sub_cat_id)
...
dfs.df[strings.AGE_GENDER] = [dfs.df[strings.GENDER][i] + dfs.df[strings.AGE][i] for i in range(len(dfs.df[strings.GENDER]))]
